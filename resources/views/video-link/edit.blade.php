        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="row g-3" action="{{ route('press-release-videos.update', 111) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        @method('Patch')
                        @for ($i = 0; $i < 3; $i++) <div class="col-5">
                            @foreach (languages() as $lng_key => $lng)

                            <div class="col-12 my-4">
                                <label for="title_{{$lng->name}}_{{ $i }}" class="form-label">Title {{
                                    Str::upper($lng->name)
                                    }}</label>
                                <input type="text"
                                    ...
                                    value='{{ 
                                        old("links.$i.title.$lng->id") ??
                                        (isset($video_link[$i]) ? $video_link[$i]->video_link_translations[$lng_key]->title : '')
                                    }}'>
                ...
                <div class="col-5 ms-5">
                    ...
                    <div class="col-12 my-4">
                        ...
                        @if ( isset($video_link[$i]) && $video_link[$i]->video_iframe != null)
                        {!! $video_link[$i]->video_iframe !!}
                        @endif
